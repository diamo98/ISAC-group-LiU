...
f = "data-new-new/o-quot-1.mat";

load(f);
[pd, sensingSinr] = computeStats(f);
yyaxis left
p1 = plot(fractionSensingUeSinr, pd, '-*', LineWidth=1.5, MarkerSize=10);
ylim([0,1]);
ylabel("Probability of detection, $P_D$", Interpreter="latex");

yyaxis right
p2 = plot(fractionSensingUeSinr, 10*log10(sensingSinr), '--*', LineWidth=1.5, MarkerSize=10);
ylim([0, 20]);
...
f = "data-new-new/n-quot-1.mat";

load(f);
[pd, sensingSinr] = computeStats(f);
yyaxis left
p3 = plot(fractionSensingUeSinr, pd, '-+', LineWidth=1.5, MarkerSize=10);
ylim([0,1]);
ylabel("Probability of detection, $P_D$", Interpreter="latex");

yyaxis right
p4 = plot(fractionSensingUeSinr, 10*log10(sensingSinr), '--+', LineWidth=1.5, MarkerSize=10);
ylim([0, 20]);
ylabel("Sensinr SINR", Interpreter="latex");
...
% combined
%f = "../new-precoder-mc-run1.mat";
f = "data-new-new/c-quot-1.mat";

load(f);
[pd, sensingSinr] = computeStats(f);
yyaxis left
p3 = plot(fractionSensingUeSinr, pd, '-x', LineWidth=1.5, MarkerSize=10);
ylim([0,1]);
ylabel("Probability of detection, $P_D$", Interpreter="latex");

yyaxis right
p4 = plot(fractionSensingUeSinr, 10*log10(sensingSinr), '--x', LineWidth=1.5, MarkerSize=10);
ylim([0, 20]);
ylabel("Sensinr SINR", Interpreter="latex");
